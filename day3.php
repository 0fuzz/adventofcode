...
function parseLine($direction, $amount, &$currentX, &$currentY, &$lineLength, &$coords, &$intersectionCoords, &$lowestdistance) {
    for ($i = 0;$i < $amount;$i++) {
        switch ($direction) {
        ...
        $lineLength++;
        if (!isset($coords[$currentX][$currentY]))
            $coords[$currentX][$currentY] = $lineLength;

        if ($intersectionCoords != null && array_key_exists($currentX, $intersectionCoords)) {
            if (array_key_exists($currentY, $intersectionCoords[$currentX]))
                $lowestdistance = min($lowestdistance,$lineLength+$intersectionCoords[$currentX][$currentY]);
        }
    } 
}

function walkLine($commands, $intersectionCoords = null)
{
    $coords = [];
    $currentX = $currentY = $lineLength = 0;
    $lowestdistance = 9999999999;
    foreach ($commands as $lineExpand) {
        $direction = $lineExpand[0];
        $amount = substr($lineExpand, 1);
        parseLine($direction, $amount, $currentX, $currentY, $lineLength, $coords, $intersectionCoords, $lowestdistance);
    }
    return ($intersectionCoords != null? $lowestdistance:$coords);
}

$lineOneCoords = walkLine($lineOne);
$shortestDistance = walkline($lineTwo, $lineOneCoords);
